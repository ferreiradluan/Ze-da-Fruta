...
import { ProdutoDomainService } from '../../domain/services/produto-domain.service';
import { PedidoDomainService } from '../../domain/services/pedido-domain.service';
import { DomainServiceIntegration } from '../../../common/domain/services/domain-service-integration';

@Injectable()
export class SalesService {
  constructor(
    // Legacy repositories for entities without rich domain implementation
    ...
    // Rich Domain Model repositories
    ...
    // Domain services
    private readonly produtoDomainService: ProdutoDomainService,
    private readonly pedidoDomainService: PedidoDomainService,
    private readonly domainServiceIntegration: DomainServiceIntegration,
  ...
  async buscarProdutosPublico(filtros: ListarProdutosDto) {
    ...
    // Use domain repository for product filtering
    const filterCriteria = {
      searchTerm: search,
      categoriaId: categoria?.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i) ? categoria : undefined,
      estabelecimentoId: estabelecimento?.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i) ? estabelecimento : undefined,
      minPrice: precoMin,
      maxPrice: precoMax,
      hasStock: true
    };

    // Use domain repository with pagination
    const result = await this.produtoRepository.findWithOptions({
      pagination: { page, limit },
      filters: [
        { field: 'ativo', operator: 'eq', value: true },
        { field: 'disponivel', operator: 'eq', value: true }
      ]
    });

    // Apply domain filters
    let filteredProducts = result.items;
    if (search || categoria || estabelecimento || precoMin !== undefined || precoMax !== undefined) {
      filteredProducts = await this.produtoRepository.findByFilters(filterCriteria);
    }
    ...
    // Manual pagination if filters were applied
