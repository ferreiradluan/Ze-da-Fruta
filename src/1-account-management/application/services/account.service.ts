...
export class AccountService {
  ...
  constructor(
    ...
    private readonly usuarioDomainService: UsuarioDomainService,
  ...
  async getMyProfile(userId: string) {
    try {
      ...
      const usuario = await this.usuarioRepository.findOne({
        where: { id: userId },
        relations: ['roles', 'perfis', 'enderecos'],
      });
      if (!usuario) {
      ...
      const perfis = Array.isArray(usuario.perfis) ? usuario.perfis : (usuario.perfis ? [usuario.perfis] : []);
      ...
      return {
        ...
        perfis: perfis.map(p => p?.descricao ?? null).filter(Boolean),
        enderecos,
      };
    } catch (err) {
  ...
  async updateUserProfilePhoto(userId: string, photoUrl: string): Promise<void> {
    ...
    try {
      ...
      const currentPhotoUrl = usuario.fotoPerfil;
  ...
  async updateAdminProfilePhoto(adminId: string, photoUrl: string): Promise<void> {
    try {
      ...
      admin.fotoPerfil = photoUrl;
      await this.adminRepository.save(admin);
      
      this.logger.log(`Foto de perfil atualizada para admin ${adminId}: ${photoUrl}`);
    } catch (err) {
      this.logger.error('Erro ao atualizar foto de perfil do admin', err);
      throw new InternalServerErrorException('Erro ao atualizar foto de perfil: ' + (err?.message || err));
    }
  }
