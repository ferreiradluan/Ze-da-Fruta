...
@Injectable()
export class GoogleEntregadorStrategy extends PassportStrategy(Strategy, 'google-entregador') {
  constructor(
  ...
  async validate(
    ...
    try {
      const user = await this.authService.autenticarComGoogle(profile, 'entregador');
      done(null, user);
    } catch (err) {
      console.error('Erro na validação Google Entregador:', err);
      done(err, false);
    }
  }
}
