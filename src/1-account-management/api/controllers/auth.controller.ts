...
export class AuthController {
  ...
  async googleAuthRedirect(@Req() req, @Res() res) {
    ...
    // Captura o parâmetro redirect (ou next) da query string
    const redirect = req.query.redirect || req.query.next || '';
    let url = `https://ze-da-futa-frony.vercel.app/auth/google/callback?token=${token}`;
    if (redirect) {
