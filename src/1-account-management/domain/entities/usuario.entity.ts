...
export class Usuario extends BaseEntity {
  ...
  rebaixarParaCliente(): void {
    ...
    this.removeRole('LOJISTA');
    this.removeRole('ENTREGADOR'); // Remove outras roles comerciais também

    // Garantir que tenha pelo menos role de cliente
    if (!this.hasRole('CLIENTE')) {
      const roleCliente = new Role();
      roleCliente.nome = 'CLIENTE';
